                    ...
                    if (savedMsg) {
                        try {
                            ...
                            if (senderJid !== myJid) {
                                ...
                                const deletedBy = senderJid;
                                const senderName = savedMsg.pushName || 'Unknown';
                                const time = new Date(savedMsg.messageTimestamp * 1000).toLocaleTimeString();
                                const groupName = savedMsg.key.remoteJid.endsWith('@g.us') ? savedMsg.key.remoteJid : null;
                                const rawMsg = savedMsg.message;
                                const content = rawMsg.conversation || rawMsg.extendedTextMessage?.text || (rawMsg.imageMessage ? '[IMAGE]' : null) || (rawMsg.videoMessage ? '[VIDEO]' : null);
                                ...
                                await sock.sendMessage(myJid, { text: text, mentions: [senderJid] });
