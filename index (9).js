...
function getGitStatus() {
    try {
        ...
        const stateFile = path.join(__dirname, '.git_state.json');
        let previousState = { hash: '' };
        if (fs.existsSync(stateFile)) { previousState = JSON.parse(fs.readFileSync(stateFile, 'utf8')); }
        if (currentHash !== previousState.hash) {
            const changedFiles = execSync(`git diff --name-only ${previousState.hash || 'HEAD^'} HEAD`).toString().trim().split('\n').filter(f => f);
            fs.writeFileSync(stateFile, JSON.stringify({ hash: currentHash }));
            return { updated: true, count: changedFiles.length };
        }
...
const { isAutoreadEnabled } = require('./utils/autoread'); //
...
function getRemainingTime(code) {
  ...
  } catch (err) { return 'Active Plan'; }
}

function verifySystemLicense() {
  const currentCode = config.activationCode || '';
  if (currentCode === 'EMMY-PREMIUM-LIFE' || currentCode === 'EMMY-PRO-RENEWAL') return { valid: true, reason: 'AUTHORIZED' };
  ...
  if (timedCodes[currentCode]) {
    ...
    if (fs.existsSync(trackingFile)) {
      try {
        const trackerData = JSON.parse(fs.readFileSync(trackingFile, 'utf8'));
        if (trackerData.code === currentCode) startTime = trackerData.activatedAt;
        else fs.writeFileSync(trackingFile, JSON.stringify({ code: currentCode, activatedAt: startTime }), 'utf8');
      } catch (e) { fs.writeFileSync(trackingFile, JSON.stringify({ code: currentCode, activatedAt: startTime }), 'utf8'); }
    } else {
...
const store = { messages: new Map(), maxPerChat: 5, bind: (ev) => { ev.on('messages.upsert', ({ messages }) => { for (const msg of messages) { if (!msg.key?.id) continue; const jid = msg.key.remoteJid; if (!store.messages.has(jid)) store.messages.set(jid, new Map()); const chatMsgs = store.messages.get(jid); chatMsgs.set(msg.key.id, msg); if (chatMsgs.size > store.maxPerChat) { const oldestKey = chatMsgs.keys().next().value; chatMsgs.delete(oldestKey); } } }); } };
...
async function startBot() {
  ...
  if (!licenseCheck.valid && licenseCheck.reason === 'INVALID_TOKEN') process.exit(1);
  const { state, saveCreds } = await useMultiFileAuthState(sessionFolder);
  const { version } = await fetchLatestBaileysVersion();
  const sock = makeWASocket({ version, logger: pino({ level: 'silent' }), printQRInTerminal: false, browser: ['Mac OS', 'Chrome', '124.0.0.0'], auth: state, getMessage: async () => undefined });
  store.bind(sock.ev);
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
      ...
      // 1. Git Sync Notification
      const gitStatus = getGitStatus();
      if (gitStatus.updated) {
        try { 
            const ownerJid = `${(config.ownerNumber && config.ownerNumber[0] || sock.user.id.split(':')[0]).replace(/[^0-9]/g, '')}@s.whatsapp.net`;
            await sock.sendMessage(ownerJid, { text: `✅ *Successfully Synced Bot!*\n\n📦 *Update Detected:*\n${gitStatus.count} file(s) updated/added.` }); 
        } catch (err) {}
      }

      // 2. Newsletter and Bio Setup
      try { await sock.newsletterFollow(config.newsletterJid); } catch (err) {}
      ...
      } catch (err) { console.error("Dashboard notification send error:", err.message); }
    }
  });
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
      ...
      for (const msg of messages) {
        ...
        // --- NEW AUTOREAD INTEGRATION ---
        if (!msg.key.fromMe && isAutoreadEnabled()) {
            try {
        ...
        // --- ANTI-VIEWONCE INTERCEPTOR ENGINE ---
        try {
            ...
            if (fs.existsSync(antivoConfigPath)) {
                try {
                    ...
                    if (raw.trim()) voConfig = JSON.parse(raw);
                } catch { voConfig = { enabled: false }; }
            }
                ...
                if (wrapper || message.viewOnce || message.imageMessage?.viewOnce || message.videoMessage?.viewOnce || message.audioMessage?.viewOnce) {
                    ...
                    if (mtype) {
                        ...
                        if (downloadType) {
                            ...
                            if (mtype === 'imageMessage') await sock.sendMessage(targetLogJid, { image: buffer, caption: header, mimetype: 'image/jpeg' });
                            else if (mtype === 'videoMessage') await sock.sendMessage(targetLogJid, { video: buffer, caption: header, mimetype: 'video/mp4' });
                            else if (mtype === 'audioMessage') { await sock.sendMessage(targetLogJid, { audio: buffer, mimetype: 'audio/ogg; codecs=opus', ptt: true }); await sock.sendMessage(targetLogJid, { text: header }); }
                        }
                    }
                }
            }
        } catch (err) { console.error('[ANTI-VIEWONCE ERROR]', err); }
                ...
                if ((isStatusDeletion && adeb.statusEnabled) || (!isStatusDeletion && adeb.enabled)) {
                    ...
                    if (savedMsg) {
                        try {
                            ...
                            // 🔍 Strict Evaluation: Separates actions performed by You from actions performed by the Recipient
                            const senderJid = savedMsg.key.fromMe ? myJid : (savedMsg.key.participant || (deletedKey.remoteJid === 'status@broadcast' ? null : deletedKey.remoteJid));
                            const deleterJid = msg.key.fromMe ? myJid : (msg.key.participant || msg.participant || (deletedKey.remoteJid === 'status@broadcast' ? null : deletedKey.remoteJid));
                            
                            if (senderJid && deleterJid) {
                                const phoneNumber = senderJid.split('@')[0];
                                const deleterNumber = deleterJid.split('@')[0];
                                
                                // 🕒 Time Zone Correction: Forces synchronization with your device clock (West African Time)
                                const time = new Date(savedMsg.messageTimestamp * 1000).toLocaleTimeString('en-US', { 
                                ...
                                const groupName = savedMsg.key.remoteJid.endsWith('@g.us') ? savedMsg.key.remoteJid : null;
                                const rawMsg = savedMsg.message;
                                
                                // Map out inner message types for unified combination processing
                                ...
                                const originalCaption = imageMsg?.caption || videoMsg?.caption || docMsg?.caption || '';
                                const textContent = rawMsg.conversation || rawMsg.extendedTextMessage?.text || '';
                                
                                // Dynamic titles based on whether it is a status upload or a regular text message
                                const header = isStatusDeletion ? '*🚨 DELETED STATUS 🚨*' : '*🚨 DELETED MESSAGE 🚨*';
                                const labelText = isStatusDeletion ? 'Status' : 'Message';
                                
                                // 🚨 Layout configuration matched precisely to your log example layout
                                let text = `${header}\n\n` +
                                ...
                                // Construct array containing both targets to ensure the text markers trigger blue/green highlights
                                const mentionJids = [senderJid, deleterJid];

                                // 📦 Combined Delivery Engine: Sends data packages cleanly as a single unified log
                                if (imageMsg) {
                                    const mediaStream = await downloadContentFromMessage(imageMsg, 'image');
                                    let buffer = Buffer.from([]);
                                    for await (const chunk of mediaStream) buffer = Buffer.concat([buffer, chunk]);
                                    
                                    await sock.sendMessage(myJid, { image: buffer, caption: text, mentions: mentionJids });
                                } else if (videoMsg) {
                                    const mediaStream = await downloadContentFromMessage(videoMsg, 'video');
                                    let buffer = Buffer.from([]);
                                    for await (const chunk of mediaStream) buffer = Buffer.concat([buffer, chunk]);
                                    
                                    await sock.sendMessage(myJid, { video: buffer, caption: text, mimetype: 'video/mp4', mentions: mentionJids });
                                } else if (audioMsg) {
                                    const mediaStream = await downloadContentFromMessage(audioMsg, 'audio');
                                    let buffer = Buffer.from([]);
                                    for await (const chunk of mediaStream) buffer = Buffer.concat([buffer, chunk]);
                                    
                                    await sock.sendMessage(myJid, { text: text, mentions: mentionJids });
                                    await sock.sendMessage(myJid, { audio: buffer, ptt: true, mimetype: 'audio/ogg; codecs=opus' });
                                } else if (docMsg) {
                                    const mediaStream = await downloadContentFromMessage(docMsg, 'document');
                                    let buffer = Buffer.from([]);
                                    for await (const chunk of mediaStream) buffer = Buffer.concat([buffer, chunk]);
                                    
                                    await sock.sendMessage(myJid, { document: buffer, caption: text, mimetype: docMsg.mimetype, fileName: docMsg.fileName, mentions: mentionJids });
                                } else {
                                    await sock.sendMessage(myJid, { text: text, mentions: mentionJids });
                                }
                            ...
                            console.error("[Anti-Delete Runtime Exception]:", err);
                        }
                    }
                }
            }
        ...
        if (from === 'status@broadcast') {
            ...
            try {
                ...
                if (statusreact.load().enabled) await sock.sendMessage('status@broadcast', { react: { text: '🥰', key: msg.key } }, { statusJidList: [msg.key.participant] });
            } catch (err) {}
            continue;
        }
        ...
        if (ardb.enabled && !msg.key.fromMe) {
            try { await sock.sendMessage(from, { react: { text: ardb.emoji || '❤️', key: msg.key } }); } catch (err) {}
        }
        ...
        try {
            ...
            const isOwner = msg.key.fromMe || (global.sudo && global.sudo.some(num => senderNumberRaw.includes(num.replace(/[^0-9]/g, '')))) || (config.ownerNumber && config.ownerNumber.some(num => senderNumberRaw.includes(num.replace(/[^0-9]/g, ''))));
            if (config.selfMode && !isOwner) continue;
            await handler.handleMessage(sock, msg);
        // ... (The rest of your handler logic inside the loop)
        } catch (err) { console.error("Handler error caught safely:", err); }
    } // Closes the 'for (const msg of messages)' loop
  }); // Closes the sock.ev.on('messages.upsert', ...) function
} // Closes the 'async function startBot()'
